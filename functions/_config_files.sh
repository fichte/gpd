...
{
	## initialize array of available asset files
	pushd "${SCRIPT_DIR}"/../docker/template/asset &>/dev/null
	ASSET_FILES_TEMPLATES=(*_template)
	ASSET_FILES_TEMPLATES_LEN=$(( "${#ASSET_FILES_TEMPLATES[@]}" - 1 ))
	popd &>/dev/null

	for t in `seq 0 "${ASSET_FILES_TEMPLATES_LEN}"`; do
		FINAL_CONFIG_FILE=$(echo "${ASSET_FILES_TEMPLATES[$t]}" | sed -e 's/_template//g')
		cp "${SCRIPT_DIR}"/../docker/template/asset/"${ASSET_FILES_TEMPLATES[$t]}" "${STACK_FINAL_CONFIG_DIR}"/asset/"${FINAL_CONFIG_FILE}"
	done
